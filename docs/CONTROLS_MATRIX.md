...
- Memory should be treated as untrusted input even when it is generated by a prior trusted session.
- Integrity checks help detect tampering, but they do not prove the content is safe.
- Provenance and content scanning reduce false negatives caused by instruction injection.
- RBAC and audit logs make the defensive story operational instead of purely conceptual.
- The same idea maps to RAG systems, copilots, customer-support agents, and long-running workflow agents.
